...
const Admin = {
    ...
    editUser(userId) {
        ...
        UI.showNotification('Edit user functionality - to be implemented', 'info');
    },
    ...
    addSalesRep(event) {
        ...
        if (result) {
            ...
            const existing = DataManager.getGlobalSalesReps().find(r => r.email && r.email.toLowerCase() === email.toLowerCase());
            if (existing) {
                UI.showNotification(`Sales user with this email already exists: ${existing.name} (${existing.email}) - Region: ${existing.region}`, 'error');
            } else {
    ...
    updateSalesRep(event, salesRepId) {
        ...
        DataManager.updateGlobalSalesRep(salesRepId, {
            name: name,
            email: email,
            region: region,
            isActive: isActive
